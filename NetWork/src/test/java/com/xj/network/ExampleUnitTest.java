...
public class ExampleUnitTest {
    ...
    public void getDataTest() {
        HttpRequestManager.getInstall().getCheckUtil().datastoreSearch("test-get-data", new HttpRequestCallback<TResponse<SourceData<Record>>>() {
            @Override
            public void callBack(TResponse<SourceData<Record>> data) {
                if (data.isSuccess()) {
                    System.out.println("拉取数据成功:" + data.getResult().getRecords().toString());
                } else {
    ...
    public void insertData() {
        ...
        Gson gson = new Gson();
        TResponse<SourceData<Record>> obj = new TResponse<>();
        TResponse<SourceData<Record>> re = gson.fromJson(value, obj.getClass());
        assertEquals(44, re.getResult().getRecords().size());
    }
}